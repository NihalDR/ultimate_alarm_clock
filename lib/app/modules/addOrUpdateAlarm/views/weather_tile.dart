...
class WeatherTile extends StatelessWidget {
    ...
    return Column(
      children: [
        ListTile(
                        ...
                        child: Padding(
                          ...
                          child: Column(
                            ...
                            children: [
                              ...
                              Text(
                                'Choose how your alarm responds to weather '
                                'conditions:',
                              ...
                              _buildInfoItem(
                                ...
                                Icons.alarm,
                                'Ring When Match',
                                'Alarm rings when weather matches selected '
                                    'types',
                              ),
                              _buildInfoItem(
                                context,
                                Icons.alarm_off,
                                'Cancel When Match',
                                'Alarm is cancelled when weather matches '
                                    'selected types',
                              ),
                              _buildInfoItem(
                                context,
                                Icons.alarm_on,
                                'Ring When Different',
                                'Alarm rings when weather is different from '
                                    'selected types',
                              ),
                              _buildInfoItem(
                                context,
                                Icons.cancel,
                                'Cancel When Different',
                                'Alarm is cancelled when weather is different '
                                    'from selected types',
                              ),
                              const SizedBox(height: 20),
                              SizedBox(
          ...
          trailing: Obx(() {
            final isWeatherEnabled = controller.weatherConditionType.value !=
                WeatherConditionType.off;
            return Switch(
              ...
              onChanged: (value) {
                ...
                if (value) {
                  controller.weatherConditionType.value =
                      WeatherConditionType.cancelWhenMatch;
                } else {
                  controller.weatherConditionType.value =
                      WeatherConditionType.off;
                }
        ...
        Obx(() {
          ...
          return AnimatedContainer(
            ...
            height: isWeatherEnabled ? null : 0,
            child: isWeatherEnabled
                ? Padding(
                    padding: const EdgeInsets.symmetric(horizontal: 16.0),
                    child: Column(
                      children: [
                        const SizedBox(height: 10),
                        ...WeatherConditionType.values
                            .where(
                              (type) => type != WeatherConditionType.off,
                            )
                            .map(
            ...
            children: [
              ...
              GestureDetector(
                onTap: () async {
                  ...
                  if (!hasPermission) {
                    Get.defaultDialog(
                      ...
                      content: const Text(
                        'Please provide all time location access to use this '
                        'feature.',
                        textAlign: TextAlign.center,
                      ),
                    );
                  ...
                  Get.defaultDialog(
                    ...
                    backgroundColor:
                        themeController.secondaryBackgroundColor.value,
                    title: 'Select weather types for '
                            '${_getWeatherConditionTitle(conditionType)}'
                        .tr,
                    titleStyle: Theme.of(context).textTheme.displaySmall,
                ...
                child: Container(
                  ...
                  child: Text(
                    controller.selectedWeather.isEmpty
                        ? 'Select Weather'
                        : '${controller.selectedWeather.length} selected',
                    style: Theme.of(context).textTheme.bodySmall?.copyWith(
                          color: Colors.black,
                          fontWeight: FontWeight.w600,
                        ),
  ...
  String _getWeatherConditionDescription(WeatherConditionType type) {
    switch (type) {
      case WeatherConditionType.ringWhenMatch:
        return 'Ring alarm when weather matches selected types '
            '(e.g., wake me when it\'s sunny)';
      case WeatherConditionType.cancelWhenMatch:
        return 'Cancel alarm when weather matches selected types '
            '(e.g., don\'t wake me when it\'s raining)';
      case WeatherConditionType.ringWhenDifferent:
        return 'Ring alarm when weather is different from selected types '
            '(e.g., wake me when it\'s not sunny)';
      case WeatherConditionType.cancelWhenDifferent:
        return 'Cancel alarm when weather is different from selected types '
            '(e.g., don\'t wake me unless it\'s raining)';
      default:
