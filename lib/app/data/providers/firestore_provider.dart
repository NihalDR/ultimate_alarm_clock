...
void _onUpgrade(Database db, int oldVersion, int newVersion) async {
    if (oldVersion < 2) {
      ...
      await db.execute(
        'ALTER TABLE alarms ADD COLUMN weatherConditionType '
    ...
    if (oldVersion < 3) {
      ...
      await db.execute(
        'ALTER TABLE alarms ADD COLUMN activityConditionType '
        'INTEGER NOT NULL DEFAULT 2',
      );
    }
    if (oldVersion < 4) {
      ...
      await db.execute(
        'ALTER TABLE alarms ADD COLUMN isSunriseEnabled '
        'INTEGER NOT NULL DEFAULT 0',
      );
      await db.execute(
        'ALTER TABLE alarms ADD COLUMN sunriseDuration '
        'INTEGER NOT NULL DEFAULT 30',
      );
      await db.execute(
        'ALTER TABLE alarms ADD COLUMN sunriseIntensity '
        'REAL NOT NULL DEFAULT 1.0',
      );
      await db.execute(
        'ALTER TABLE alarms ADD COLUMN sunriseColorScheme '
        'INTEGER NOT NULL DEFAULT 0',
      );
    }
    if (oldVersion < 5) {
      ...
      await db.execute(
        'ALTER TABLE alarms ADD COLUMN timezoneId '
        'TEXT NOT NULL DEFAULT ""',
      );
      await db.execute(
        'ALTER TABLE alarms ADD COLUMN isTimezoneEnabled '
        'INTEGER NOT NULL DEFAULT 0',
      );
      await db.execute(
        'ALTER TABLE alarms ADD COLUMN targetTimezoneOffset '
        'INTEGER NOT NULL DEFAULT 0',
      );
    }
    if (oldVersion < 6) {
      ...
      await db.execute(
        'ALTER TABLE alarms ADD COLUMN smartControlCombinationType '
        'INTEGER NOT NULL DEFAULT 0',
      );
    }
  }
  ...
  static CollectionReference _alarmsCollection(UserModel? user) {
    // Always use sharedAlarms collection since we're using shared
    // alarm functionality
  ...
  static Future<void> addUser(UserModel userModel) async {
    try {
      ...
      debugPrint(
          '   - Firebase Auth User: '
          '${_firebaseAuthInstance.currentUser?.uid},');
      ...
      if (!user.exists) {
        ...
        debugPrint(
            '✅ Created new user document with receivedItems: '
            '${userModel.id},');
      } else {
        ...
        if (data != null && !data.containsKey('receivedItems')) {
          ...
          debugPrint(
              '✅ Added receivedItems field to existing user: '
              '${userModel.id},');
        } else {
          debugPrint(
              '✅ User document already exists with receivedItems: '
              '${userModel.id},');
        }
      }
    } catch (e) {
  ...
  static addAlarm(UserModel? user, AlarmModel alarmRecord) async {
    ...
    if (alarmRecord.isSharedAlarmEnabled) {
      ...
      debugPrint(
          '✅ Created shared alarm in Firestore: '
      ...
      try {
        ...
        if (e.toString().contains('locationConditionType') ||
          ...
          await sql!.insert('alarms', fallbackMap).then(
              (value) => debugPrint('insert success (backward compatibility)'),
            );
        } else {
    ...
    try {
      ...
      if (alarmDoc.exists) {
        ...
        if (offsetDetailsRaw is Map) {
          ...
          acceptedUsers.addAll(offsetDetails.keys);
        ...
        debugPrint(
            '   - acceptedUsers (from offsetDetails): '
            '${acceptedUsers.toList()},');
        ...
        if (dismissedByUsers.length >= allUsers.length) {
          await FirebaseFirestore.instance
              ...
              .delete();
          debugPrint(
              '✅ All users who have the alarm dismissed it - deleted '
              'shared alarm completely: $firestoreId,');
        } else {
          debugPrint(
              '⏳ Not all users who have the alarm have dismissed yet - '
              'keeping alarm: $firestoreId,');
        }
      }
    } catch (e) {
  ...
  static Stream<QuerySnapshot<Object?>> getSharedAlarms(UserModel? user) {
    if (user != null) {
      ...
      // Return a stream that emits from an empty collection
      // to provide a proper QuerySnapshot.
  ...
  static Future<List<String>> removeUserFromAlarmSharedUsers(
    ...
    final rawOffsetDetails = alarmDoc.data()['offsetDetails'];
    Map<String, dynamic> offsetDetails = {};
    if (rawOffsetDetails is Map) {
      offsetDetails = Map<String, dynamic>.from(rawOffsetDetails);
    } else if (rawOffsetDetails is List) {
      for (final item in rawOffsetDetails) {
        if (item is Map && item.containsKey('userId')) {
          ...
          if (id.isNotEmpty) {
            offsetDetails[id] = {
              ...
              'offsettedTime':
                  item['offsettedTime'] ?? alarmDoc.data()['alarmTime'],
            };
          }
        }
      }
    ...
    final dismissedByUsers =
        List<String>.from(alarmDoc.data()['dismissedByUsers'] ?? []);

    if (sharedUserIds.contains(userModelId)) {
      sharedUserIds.remove(userModelId); // Remove the userId from the list
      offsetDetails.remove(userModelId);
      dismissedByUsers.remove(userModelId);

      await alarmDoc.reference.update({
        'sharedUserIds': sharedUserIds,
        'offsetDetails': offsetDetails,
        'dismissedByUsers': dismissedByUsers,
      });
    }
  ...
  static acceptSharedAlarm(String alarmOwnerId, AlarmModel alarm) async {
    ...
    if (alarmDoc.exists) {
      ...
      debugPrint(
          '✅ User $currentUserId accepted shared alarm and added '
          'to offsetDetails,');
    }
  }
  ...
  static Future<void> triggerRescheduleUpdate(AlarmModel alarmData) async {
    try {
      ...
      debugPrint(
          '✅ Triggered Firestore update for shared alarm reschedule: '
  ...
  static Future<void> releaseSharedAlarmLock(String firestoreId) async {
    try {
      await _firebaseFirestore
          .collection('sharedAlarms')
          .doc(firestoreId)
      ...
      debugPrint('✅ Released shared alarm lock: $firestoreId,');
    } catch (e) {
      debugPrint('⚠️ Failed to release shared alarm lock: $e');
    }
  }
}
